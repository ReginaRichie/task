...
class ShopUnit(BaseModel):
    id: UUID = Field(..., description='Уникальный идентфикатор', example='3fa85f64-5717-4562-b3fc-2c963f66a333')
    name: str = Field(..., description='Имя категории')
    date: datetime = Field(..., description='Время последнего обновления элемента.', example='2022-05-28T21:12:01.000Z')
    parentId: Optional[UUID] = Field(None, description='UUID родительской категории',
    ...
    children: Optional[List[ShopUnit]] = Field(None,
...
class ShopUnitImportRequest(BaseModel):
    items: Optional[List[ShopUnitImport]] = Field(None, description='Импортируемые элементы')
    updateDate: Optional[datetime] = Field(None, description='Время обновления добавляемых товаров/категорий.',
